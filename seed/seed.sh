...
ensure_agent_avatar "glm-maintainer" "/assets/agent-avatars/glm-maintainer.png"
ensure_agent_avatar "designer-agent" "/assets/agent-avatars/designer-agent.png"
ensure_agent_avatar "coding-agent" "/assets/agent-avatars/coding-agent.png"
ensure_agent_avatar "docs-agent" "/assets/agent-avatars/docs-agent.png"
ensure_agent_avatar "review-agent" "/assets/agent-avatars/review-agent.png"
...
# ------------------------------------------------------------------------
# Git-backed knowledge library. Publications keep Markdown entries separated
# by purpose in one repository per person or team. OpenFace aggregates
# articles/, procedures/, and wiki/ from public repositories with `doc`.
# ------------------------------------------------------------------------
...
EOF
create_doc_fixture "catalog-topic-reference" "OpenFaceの各一覧で共通して使うトピック仕様" "wiki" "catalog" "metadata" "${WORKDIR}/doc_catalog_reference.md"
...
用途に応じて`articles/my-first-note.md`、`procedures/setup.md`、`wiki/glossary.md`のいずれかを作り、フロントマターへ記事情報を書きます。
...
`/docs`へ戻り、形式を選んでナレッジを開きます。表示されない場合は、リポジトリが公開されていること、`doc`トピックがあること、ファイルが`articles/`、`procedures/`、`wiki/`のいずれかにあること、`published`が`false`でないことを確認します。
EOF
...
EOF
create_doc_fixture "tailscale-private-deployment" "信頼できるtailnet端末へOpenFaceを安全に公開する" "procedure" "tailscale" "deployment" "${WORKDIR}/doc_tailscale_guide.md"
...
EOF
create_doc_fixture "incident-recovery-playbook" "OpenFaceの一般的な障害を安全に診断・復旧する手順" "procedure" "recovery" "operations" "${WORKDIR}/doc_recovery_playbook.md"
...
EOF
create_doc_fixture "document-format-reference" "文書に合う形式と検索しやすいトピックの選び方" "wiki" "docs" "taxonomy" "${WORKDIR}/doc_format_reference.md"
