...
`glm-maintainer` automatically delegates each request to one specialist: `@designer-agent`, `@coding-agent`, `@docs-agent`, or `@review-agent`. The delegation is a real, visible conversation step: the maintainer first posts `@specialist 次の作業を担当してください`, and only then is that specialist's worker submitted. Users always address the maintainer; direct specialist mentions neither start a run nor override its routing. Each specialist has its own Forgejo identity, avatar, least-privilege token, role contract, commit author, reactions, and completion reply; `/api/agents` and `/api/jobs` expose the available personas and current assignment.
...
The service validates the Forgejo HMAC signature and deduplicates deliveries in SQLite. Claude Code runs as an unprivileged user inside the maintenance container: it has no host Docker socket and cannot read the Forgejo bot token, while retaining normal repository-level tools and test execution. The root wrapper alone commits and pushes after `git diff --check`. With the Compose default `MAINTENANCE_AUTO_MERGE=true`, it then requests a server-side Forgejo merge and branch deletion; a failed validation, merge conflict, or rejected merge leaves the job failed and inspectable. Set the variable to `false` to restore human-review-only PRs. See [Automated Claude Code maintenance](https://sunwood-ai-labs.github.io/OpenFace/guide/automated-maintenance).

UI and application changes have an additional evidence gate. The specialist must run the real app, list the interactions and browser checks it performed, and attach real mobile (≤480px) and desktop (≥1024px) PNG screenshots. The wrapper rejects failed tests, missing or invalid images, or missing viewport coverage before publication. It then uploads the screenshots and a Markdown UI-test table to the specialist's completion comment. Chromium, Japanese CJK fonts, and color emoji are included in the maintenance image so the captured Japanese UI remains readable.
