...
<img src="docs/images/seraphim-labs-team-mobile.png" alt="4名の異なる架空の天使キャラクターを表示したSeraphim Labsチーム" width="390">
...
内部の [`/docs`](https://localhost:8443/docs) は、運用マニュアル用VitePressとは別の、Gitリポジトリを土台にしたナレッジカテゴリです。topicへ `doc` を追加し、Markdownは整理用に `articles/`、`procedures/`、`wiki/`、`news/`へ保存します。表示上の役割は複合指定でき、`formats: [news, article]`ならひとつの更新情報がニュース兼読みものになります。frontmatterの`emoji`は記事の識別アイコンとカード背景の薄い透かしになり、未指定時はトピックに応じた絵文字を安定して自動選択します。実ファイル、コミット履歴、clone URL、権限はそのまま保持され、実閲覧数によるトレンド順とタグ単位の閲覧を利用できます。
...
| 一覧の連携状態 | 根拠付きサイドバー |
|---|---|
| <img src="docs/evidence/skill-relationships/skills-desktop.png" alt="連携数とStandalone状態を表示するSkills一覧" width="100%"> | <img src="docs/evidence/skill-relationships/graph-desktop.png" alt="SKILL.md根拠を表示するSkill関係サイドバー" width="100%"> |
...
追跡対象外の `.env` で `ZAI_AGENT_CONFIG` にZ.AIの保護されたenvファイルを指定し、`maintenance-agent` を起動します。`openface` 組織でIssueが新規作成されると、Claude Code 2.1.205の組み込み `/goal` がIssueを完了条件として受け取ります。Z.AIのAnthropic互換endpoint経由で `glm-5.2` を使い、clone内を自由に調査・編集・テストした結果を `agent/issue-N` ブランチから人間レビュー前提のPRにします。プロンプト、実行結果、PR本文、Issue返信は日本語です。`agent:skip` ラベルまたは `<!-- openface-maintenance:skip -->` で除外できます。
...
実際の引き継ぎは [Issue #21](https://madesk.tail8be30.ts.net/git/openface/pages-starter/issues/21) → [PR #22](https://madesk.tail8be30.ts.net/git/openface/pages-starter/pulls/22) として保存しています。`glm-maintainer` のメンション → `docs-agent` のリアクション・作業 → `docs-agent` の完了コメント、という順序を確認できます。

![GLM Maintainerが専門担当をメンションしてから担当が作業・返信する会話](docs/evidence/agents/maintainer-delegates-specialist-complete.png)
