...
追跡対象外の `.env` で `OPENWEBUI_AGENT_CONFIG` に既存のOpen WebUIエージェント設定を指定し、`maintenance-agent` を起動します。`openface` 組織でIssueが新規作成されると、Claude Code 2.1.205の組み込み `/goal` がIssueを完了条件として受け取ります。ローカルの `glm-4.7` を使い、clone内を自由に調査・編集・テストした結果を `agent/issue-N` ブランチから人間レビュー前提のPRにします。`agent:skip` ラベルまたは `<!-- openface-maintenance:skip -->` で除外できます。
...
実E2Eは [Issue #10](https://madesk.tail8be30.ts.net/git/openface/pages-starter/issues/10) → [PR #11](https://madesk.tail8be30.ts.net/git/openface/pages-starter/pulls/11) として残しています。Claude Code `/goal` が `glm-4.7` で `README.md` と `index.html` を変更し、`glm-maintainer` がIssueへ逆リンクを投稿し、merge可能なPRを作成したことを確認済みです。
