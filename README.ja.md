...
PR作成後も、元Issueまたはエージェントが作ったPRへ `/goal 見出しも日本語にしてください。` のようにコメントすると追加編集できます。同じ `agent/issue-N` ブランチをcloneし、追加指示を実装・検証して、同じPRへ新しいcommitをpushします。通常のコメントでは起動せず、先頭が `/goal` のコメントだけが実行対象です。
