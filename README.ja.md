...
追跡対象外の `.env` で `OPENWEBUI_AGENT_CONFIG` に既存のOpen WebUIエージェント設定を指定し、`maintenance-agent` を起動します。`openface` 組織でIssueが新規作成されると、ローカルの `glm-4.7` が planner・coder・reviewer の3段階で処理し、`agent/issue-N` ブランチから人間レビュー前提のPRを作成します。`agent:skip` ラベルまたは `<!-- openface-maintenance:skip -->` で除外できます。

署名検証、Issue単位の重複排除、推論前の秘密らしい値のマスク、禁止パス、変更ファイル数・行数上限、静的構文検査をfail-closedで実施します。エージェント自身はマージしません。詳細は[GLM自動メンテナンス](docs/ja/guide/automated-maintenance.md)を参照してください。
