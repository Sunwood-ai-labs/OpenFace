...
export async function generateMetadata({ params }: { params: Promise<{ owner: string; slug: string }> }) {
  ...
  return { title: article ? `${article.title} - ナレッジ - OpenFace` : 'ナレッジ - OpenFace', description: article?.description };
}
    ...
    <article className="openface-knowledge-article min-h-screen">
      <div className="openface-article-author mx-auto flex max-w-[920px] items-center justify-between px-5 py-4 sm:px-8">
        <Link href="/docs" className="inline-flex items-center gap-2 text-xs font-bold text-teal-800 hover:underline dark:text-teal-300">
          <HfIcon name="arrowLeft" className="h-3 w-3" /> 記事一覧
        </Link>
      ...
      <header className="openface-article-hero border-y border-zinc-200 px-5 py-12 text-center dark:border-zinc-800 sm:px-8 sm:py-16">
        ...
        <p className="mt-6 text-xs text-zinc-500 dark:text-zinc-400">{timeAgoJa(article.updatedAt)}に更新 · 読了 {article.readingMinutes}分</p>
      </header>
      <div className="mx-auto max-w-[1040px] px-5 sm:px-8">
        ...
        <div className="grid gap-10 py-10 lg:grid-cols-[minmax(0,760px)_190px] lg:justify-center lg:py-14">
          ...
          <p className="font-mono text-[10px] font-bold uppercase tracking-[0.16em] text-zinc-500">出典</p>
          <p className="mt-3 leading-6 text-zinc-600 dark:text-zinc-400">この記事は <code>articles/{article.slug}.md</code> から公開されています。</p>
            <a href={`${forgejoRepoUrl(article.owner, article.repository)}/src/branch/${article.branch}/${article.path}`} className="mt-4 inline-flex items-center gap-2 font-bold text-teal-900 hover:underline dark:text-teal-300">ソースを見る <HfIcon name="arrowRight" className="h-3 w-3" /></a>
          </aside>
        </div>
      </div>
    </article>
