...
import { timeAgoJa } from '@/lib/format';
...
const formats: Array<{
  ...
  { value: 'article', label: '記事', description: '背景、意思決定、調査や実践の記録', icon: 'filePen' },
  { value: 'wiki', label: 'Wiki', description: '概念同士のつながりとシステムマップ', icon: 'model' },
  { value: 'guide', label: 'ガイド', description: '最初から最後まで実行できる手順', icon: 'arrowRight' },
  { value: 'reference', label: 'リファレンス', description: '仕様、契約、用語などの安定した情報', icon: 'file' },
];
    ...
    <article className="openface-doc-row group relative grid min-w-0 grid-cols-[68px_minmax(0,1fr)] gap-4 border-t border-zinc-200 py-5 transition-colors dark:border-zinc-800 sm:grid-cols-[80px_minmax(0,1fr)_auto] sm:gap-5 sm:px-2">
      ...
      <div className="min-w-0">
        <div className="mb-1.5 flex flex-wrap items-center gap-2 text-[10px] font-bold uppercase tracking-[0.14em]">
          ...
          <span className="text-zinc-500 dark:text-zinc-400">読了 {article.readingMinutes}分</span>
        </div>
        ...
        <p className="mt-2 text-xs text-zinc-500 dark:text-zinc-400"><strong className="text-zinc-700 dark:text-zinc-300">{article.owner}</strong> · {timeAgoJa(article.updatedAt)}</p>
        {topics.length > 0 ? (
      ...
      <Link href={docHref(article)} aria-label={`${article.title}を読む`} className="hidden h-9 items-center gap-2 self-center px-3 text-xs font-bold text-zinc-500 transition hover:text-teal-800 dark:text-zinc-400 dark:hover:text-teal-300 sm:inline-flex">
        読む <HfIcon name="arrowRight" className="h-3 w-3" />
      </Link>
    </article>
  );
}
  ...
  return (
    <div className="openface-docs-directory min-h-screen bg-[#fbfaf6] text-zinc-950 dark:bg-zinc-950 dark:text-zinc-100">
        ...
        <div className="openface-docs-hero-grid relative mx-auto grid max-w-[1440px] gap-8 px-5 py-12 sm:px-8 sm:py-16 lg:grid-cols-[minmax(0,1fr)_340px] lg:items-end lg:px-12">
          <div>
            <div className="mb-5 flex items-center gap-3 font-mono text-[11px] font-bold uppercase tracking-[0.22em] text-orange-700 dark:text-orange-400">
              <span className="h-px w-10 bg-current" /> OpenFace ナレッジライブラリ
            </div>
            ...
            <p className="openface-docs-hero-copy mt-7 max-w-2xl text-base leading-7 text-zinc-600 dark:text-zinc-400 sm:text-lg">
              記事、調査メモ、手順書、長く使えるリファレンスを、あなたやチームのGitリポジトリから公開できます。
            </p>
            <div className="openface-docs-hero-actions mt-7 flex flex-wrap gap-3">
              <a href="#how-to-use" className="inline-flex h-11 items-center gap-2 border border-zinc-950 bg-zinc-950 px-5 text-sm font-bold text-white transition hover:bg-teal-900 dark:border-zinc-100 dark:bg-zinc-100 dark:text-zinc-950 dark:hover:border-teal-300 dark:hover:bg-teal-300">
                ナレッジを公開する <HfIcon name="arrowRight" className="h-3.5 w-3.5" />
              </a>
          ...
          <dl className="openface-docs-hero-stats grid grid-cols-2 border-y border-zinc-300 py-5 dark:border-zinc-700 lg:grid-cols-1 lg:border-y-0 lg:border-l lg:py-0 lg:pl-8">
            <div className="border-r border-zinc-200 pr-5 dark:border-zinc-800 lg:border-b lg:border-r-0 lg:pb-5 lg:pr-0">
              <dt className="font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-zinc-500">公開中の記事</dt>
              <dd className="mt-2 font-serif text-4xl">{result.ok ? result.totalCount : 0}</dd>
            </div>
            <div className="pl-5 lg:pt-5 lg:pl-0">
              <dt className="font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-zinc-500">登録トピック</dt>
      ...
      <div className="mx-auto max-w-[1440px] px-5 py-8 sm:px-8 lg:px-12">
        ...
        {!result.ok ? (
          <div className="border border-dashed border-zinc-300 p-12 text-center text-zinc-500 dark:border-zinc-700">Forgejoに接続できませんでした。</div>
        ) : docs.length === 0 ? (
          <div className="border border-dashed border-zinc-300 p-12 text-center dark:border-zinc-700">
            ...
            <p className="mt-4 font-serif text-2xl">条件に一致する記事がありません</p>
            <Link href="/docs" className="mt-3 inline-block text-sm font-semibold text-teal-800 underline dark:text-teal-300">絞り込みを解除</Link>
          </div>
        ) : (
          <>
            {featured ? (
              <section className="openface-doc-feature mb-12 grid overflow-hidden rounded-3xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900 lg:grid-cols-[minmax(0,1fr)_280px]">
                <div className="p-6 sm:p-9 lg:p-10">
                  <p className="font-mono text-[11px] font-bold uppercase tracking-[0.2em] text-orange-700 dark:text-orange-400">注目 / {formatLabel(featured.format)}</p>
                  ...
                  <p className="mt-3 font-mono text-[10px] uppercase tracking-wide text-zinc-400">{featured.owner} が {featured.repository} から公開</p>
                  <Link href={docHref(featured)} className="openface-docs-primary-action mt-8 inline-flex h-11 items-center gap-3 bg-zinc-950 px-5 text-sm font-bold text-white transition hover:bg-teal-900 dark:bg-zinc-100 dark:text-zinc-950 dark:hover:bg-teal-300">
                    記事を開く <HfIcon name="arrowRight" className="h-3.5 w-3.5" />
                  </Link>
                </div>
            ...
            <section className="grid gap-10 lg:grid-cols-[260px_minmax(0,1fr)]">
              <aside>
                <p className="mb-5 font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-500">形式から探す</p>
                ...
                <Link href="/new?type=doc" className="mt-6 inline-flex items-center gap-2 text-sm font-bold text-teal-900 hover:underline dark:text-teal-300">
                  <HfIcon name="plus" className="h-3.5 w-3.5" /> ナレッジを追加
                </Link>
              </aside>
              ...
              <div className="min-w-0">
                <div className="mb-2 flex items-end justify-between gap-4">
                  <div>
                    <p className="font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-orange-700 dark:text-orange-400">ナレッジ一覧</p>
                    <h2 className="mt-2 font-serif text-4xl">{selectedFormat ? formatLabel(selectedFormat) : q ? `「${q}」の検索結果` : '新着を読む'}</h2>
                  </div>
                  <span className="font-mono text-xs text-zinc-500">{docs.length}件</span>
                </div>
                <div>
        ...
        <section id="how-to-use" aria-labelledby="docs-how-to-title" className="openface-docs-onboarding mt-14 scroll-mt-24 border-y border-zinc-300 bg-[#f4f0e8] dark:border-zinc-700 dark:bg-zinc-900">
          <div className="grid lg:grid-cols-[220px_minmax(0,1fr)]">
            <div className="border-b border-zinc-300 p-6 dark:border-zinc-700 lg:border-b-0 lg:border-r lg:p-8">
              <p className="font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-orange-700 dark:text-orange-400">ここから始める</p>
              <h2 id="docs-how-to-title" className="mt-3 font-serif text-3xl leading-none">3ステップでナレッジを公開。</h2>
              <p className="mt-4 text-sm leading-6 text-zinc-600 dark:text-zinc-400">個人またはチームごとに1つの公開リポジトリを持ち、articles/内のMarkdownを記事として配信します。</p>
            </div>
            <div className="grid sm:grid-cols-3">
              <div className="border-b border-zinc-300 p-6 dark:border-zinc-700 sm:border-b-0 sm:border-r lg:p-8">
                ...
                <h3 className="mt-7 font-serif text-2xl">書き残す</h3>
                <p className="mt-2 text-sm leading-6 text-zinc-600 dark:text-zinc-400">公開リポジトリを一度作り、記事、調査メモ、手順書、資料を <code className="bg-white px-1 py-0.5 text-xs dark:bg-zinc-950">articles/*.md</code> に保存します。</p>
              </div>
              <div className="border-b border-zinc-300 p-6 dark:border-zinc-700 sm:border-b-0 sm:border-r lg:p-8">
                ...
                <h3 className="mt-7 font-serif text-2xl">整理する</h3>
                <p className="mt-2 text-sm leading-6 text-zinc-600 dark:text-zinc-400">フロントマターでタイトル、形式、トピックを指定します。検索とトピックリンクから、あとで必要な知識へ戻れます。</p>
              </div>
              <div className="p-6 lg:p-8">
                ...
                <h3 className="mt-7 font-serif text-2xl">育てる</h3>
                <p className="mt-2 text-sm leading-6 text-zinc-600 dark:text-zinc-400">考えが変わったらMarkdownを更新します。コミットが履歴を残し、公開前にはプルリクエストでレビューできます。</p>
                <Link href="/new?type=doc&template=documentation" className="mt-4 inline-flex items-center gap-2 text-sm font-bold text-teal-900 hover:underline dark:text-teal-300">ナレッジを追加 <HfIcon name="arrowRight" className="h-3 w-3" /></Link>
              </div>
            </div>
          </div>
        </section>
