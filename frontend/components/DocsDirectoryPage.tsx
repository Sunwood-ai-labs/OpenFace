      ...
      <div className={`relative flex min-h-32 items-start justify-between overflow-hidden bg-gradient-to-br p-5 ${coverTone}`}>
        ...
        <span className="relative flex flex-wrap gap-1.5">
          {article.formats.map((value) => (
            <span key={value} className="rounded-full border border-current/20 bg-white/65 px-2.5 py-1 font-mono text-[10px] font-bold uppercase tracking-[0.14em] backdrop-blur-sm">
...
export default async function DocsDirectoryPage({
  ...
  const docs = selectedFormat
    ? result.data.filter((article) => article.formats.includes(selectedFormat))
    : selectedTag
  ...
  const sortedDocs = sort === 'trending'
    ? [...docs].sort((left, right) => right.views - left.views || right.updatedAt.localeCompare(left.updatedAt))
    : [...docs].sort((left, right) => right.updatedAt.localeCompare(left.updatedAt));
  const featured = trending.find((article) => docs.some((doc) => doc.id === article.id)) || docs[0];
  const remaining = featured ? sortedDocs.filter((article) => article.id !== featured.id) : sortedDocs;
  ...
  const formatHref = (value?: DocFormat, sortValue: DocSort = sort) => {
    const params = new URLSearchParams();
    if (q) params.set('q', q);
    if (sortValue !== 'updated') params.set('sort', sortValue);
        ...
        <div className="openface-docs-hero-grid relative mx-auto grid max-w-[1440px] gap-8 px-5 py-12 sm:px-8 sm:py-16 lg:grid-cols-[minmax(0,1fr)_340px] lg:items-end lg:px-12">
          <div>
            ...
            <p className="openface-docs-hero-copy mt-7 max-w-2xl text-base leading-7 text-zinc-600 dark:text-zinc-400 sm:text-lg">
              {ui(locale, '読みものの「記事」、再現できる「手順」、育ち続ける「Wiki」。ひとつのナレッジに複数の役割を付けて届けます。', 'Publish one piece of knowledge with any combination of article, procedure, and living Wiki roles.')}
            </p>
      ...
      <div className="mx-auto max-w-[1440px] px-5 py-8 sm:px-8 lg:px-12">
        <div className="mb-9 grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
          ...
          <div className="flex flex-wrap gap-2">
            <Link href={formatHref(selectedFormat, 'updated')} aria-current={sort === 'updated' ? 'page' : undefined} className={`openface-docs-format-tab border px-3 py-2 text-xs font-bold uppercase tracking-wide ${sort === 'updated' ? 'openface-docs-format-active border-zinc-950 bg-zinc-950 text-white dark:border-zinc-100 dark:bg-zinc-100 dark:text-zinc-950' : 'border-zinc-300 text-zinc-600 hover:border-zinc-500 dark:border-zinc-700 dark:text-zinc-300'}`}>
              {ui(locale, '新着', 'Latest')}
            </Link>
            <Link href={formatHref(selectedFormat, 'trending')} aria-current={sort === 'trending' ? 'page' : undefined} className={`openface-docs-format-tab border px-3 py-2 text-xs font-bold uppercase tracking-wide ${sort === 'trending' ? 'openface-docs-format-active border-orange-700 bg-orange-700 text-white dark:border-orange-300 dark:bg-orange-300 dark:text-zinc-950' : 'border-zinc-300 text-zinc-600 hover:border-zinc-500 dark:border-zinc-700 dark:text-zinc-300'}`}>
              {ui(locale, 'トレンド', 'Trending')}
            </Link>
            <span className="mx-1 hidden h-8 w-px bg-zinc-200 dark:bg-zinc-800 sm:block" aria-hidden="true" />
            ...
            {featured ? (
              <section className="openface-doc-feature mb-12 grid overflow-hidden rounded-3xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900 lg:grid-cols-[minmax(0,1fr)_280px]">
                <div className="p-6 sm:p-9 lg:p-10">
                  <p className="flex flex-wrap items-center gap-1.5 font-mono text-[11px] font-bold uppercase tracking-[0.2em] text-orange-700 dark:text-orange-400">
                    <span>{ui(locale, 'いま読まれている', 'Trending now')}</span>
                    {featured.formats.map((value) => <span key={value}>/ {formatLabel(value, formats)}</span>)}
                  </p>
            ...
            <section className="grid gap-10 lg:grid-cols-[260px_minmax(0,1fr)]">
              <aside>
                ...
                <div className="grid gap-0 border-t border-zinc-300 dark:border-zinc-700">
                  {formats.map((format) => {
                    const count = result.data.filter((article) => article.formats.includes(format.value)).length;
                    return (
              ...
              <div className="min-w-0">
                <div className="mb-2 flex items-end justify-between gap-4">
                  <div>
                    ...
                    <h2 className="mt-2 font-serif text-4xl">{selectedFormat ? formatLabel(selectedFormat, formats) : selectedTag ? `#${selectedTag}` : q ? ui(locale, `「${q}」の検索結果`, `Results for “${q}”`) : sort === 'trending' ? ui(locale, 'よく読まれている', 'Trending knowledge') : ui(locale, '新着を読む', 'Latest knowledge')}</h2>
                  </div>
          ...
          <div className="grid lg:grid-cols-[220px_minmax(0,1fr)]">
            ...
            <div className="grid sm:grid-cols-3">
              <div className="border-b border-zinc-300 p-6 dark:border-zinc-700 sm:border-b-0 sm:border-r lg:p-8">
                ...
                <p className="mt-2 text-sm leading-6 text-zinc-600 dark:text-zinc-400">{ui(locale, '保存先は整理用です。フロントマターの', 'Folders organize files. Use')} <code className="bg-white px-1 py-0.5 text-xs dark:bg-zinc-950">formats: [article, wiki]</code>{ui(locale, ' のように、記事・手順・Wikiを自由に組み合わせられます。', ' in front matter to combine article, procedure, and Wiki roles freely.')}</p>
              </div>
              <div className="border-b border-zinc-300 p-6 dark:border-zinc-700 sm:border-b-0 sm:border-r lg:p-8">
