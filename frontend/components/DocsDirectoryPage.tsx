    ...
    <article className="group relative min-w-0 border-t border-zinc-200 py-5 transition-colors hover:bg-[#f4f0e8] dark:border-zinc-800 dark:hover:bg-zinc-900/80 sm:grid sm:grid-cols-[48px_minmax(0,1fr)_auto] sm:gap-4 sm:px-3">
      ...
      <div className="min-w-0">
        <div className="mb-2 flex flex-wrap items-center gap-2 text-[11px] font-bold uppercase tracking-[0.14em]">
          ...
          <span className="text-zinc-500 dark:text-zinc-400">Updated {timeAgoEn(article.updatedAt)}</span>
        </div>
        <Link href={docHref(article)} className="block font-serif text-2xl font-semibold leading-tight text-zinc-950 transition group-hover:text-teal-900 dark:text-zinc-100 dark:group-hover:text-teal-200">
          {article.title}
        </Link>
        <p className="mt-2 max-w-3xl text-sm leading-6 text-zinc-600 dark:text-zinc-400">
          {article.description}
        </p>
        <p className="mt-2 font-mono text-[10px] uppercase tracking-wide text-zinc-400">by {article.owner}</p>
        {topics.length > 0 ? (
      ...
      <Link href={docHref(article)} aria-label={`Read ${article.title}`} className="mt-4 inline-flex h-9 items-center gap-2 self-center border border-zinc-300 px-3 text-xs font-bold uppercase tracking-wide text-zinc-700 transition hover:border-teal-800 hover:bg-teal-900 hover:text-white dark:border-zinc-700 dark:text-zinc-300 dark:hover:border-teal-300 dark:hover:bg-teal-300 dark:hover:text-zinc-950 sm:mt-0">
