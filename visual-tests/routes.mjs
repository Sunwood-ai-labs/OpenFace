...
export const routes = [
  ...
  { id: 'character-purupuru-detail', label: 'PuruPuru character detail', path: '/openface/lumi-jelly-head-motion-pngtuber', scrollSelectors: [{ id: 'format-panel', selector: 'section img[alt*="キャラクタープレビュー"], section img[alt*="character preview"]' }], focus: 'Five-direction 30-state detection, motion patch link, imported README, and format evidence' },
  { id: 'character-pet-detail', label: 'Codex Pet collection detail', path: '/openface/character-design-images', scrollSelectors: [{ id: 'format-panel', selector: 'section img[alt*="キャラクタープレビュー"], section img[alt*="character preview"]' }], focus: 'Eight paired pet packages, Maki assets, character sheets, QA evidence, and preview rendering' },
  ...
  { id: 'community-reactions', label: 'Community reactions', path: '/git/openface/pages-starter/issues/1', scrollSelectors: [{ id: 'reaction-pills', selector: '.ui.attached.segment.reactions' }], focus: 'Independent Hugging Face-style reaction pills across themes in a fixture that exists after every clean Compose seed' },
  { id: 'pull-detail', label: 'Pull request detail', path: '/git/openface/pages-starter/pulls/2', scrollSelectors: [{ id: 'merge-instructions', selector: '.merge-section' }], focus: 'Hugging Face-style pull request title, branch metadata, Discussion and Files changed navigation, commit-history metadata, Markdown comment body, timeline events, merge instructions, and mobile theme contrast' },
  { id: 'pull-commits', label: 'Pull request commits', path: '/git/openface/pages-starter/pulls/2/commits', focus: 'Pull request commit history, active-tab visibility, responsive spacing, and theme contrast' },
  { id: 'pull-files', label: 'Pull request changed files', path: '/git/openface/pages-starter/pulls/2/files', scrollSelectors: [{ id: 'diff-body', selector: '.diff-file-box .file-body' }], focus: 'Pull request changed-file toolbar, file headers, contained horizontal code scrolling, and theme contrast' },
